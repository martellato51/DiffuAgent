...
def optional_max_memory(value: str | None) -> dict[int | str, str] | None:
    if value is None or value.lower() in {"", "none", "null"}:
        return None
    ...
    for idx, item in enumerate(value.split(",")):
        ...
        if ":" in item:
            ...
            device = device.strip()
            key: int | str = "cpu" if device == "cpu" else int(device)
        else:
            key = idx
            memory = item
        max_memory[key] = memory.strip()
...
class TraceableLLaDABackend:
    def __init__(self, dtype: str = "bfloat16") -> None:
        ...
        self.model = LLaDAModelLM.from_pretrained(
            ...
            device_map="auto",
            max_memory=optional_max_memory(os.getenv("LLADA_MAX_MEMORY")),
        ).eval()
